...
for file in file_list:
    ...
    cmd_tmp = cmd_run + [msconvert_path] + ['--'] + ['msconvert/arg0', '--outdir', tmp_dir.replace('/','\\'), '-v', '--mzXML', '--filter', '\"peakPicking true 1-\"', guest_file.replace('/','\\')]
...
#    cmd_tmp = cmd_run + [msconvert_path] + ['--'] + ['msconvert/arg0', '--outdir', tmp_dir.replace('/','\\'), '-v', guest_file.replace('/','\\')]
    ...
    gfile = gbase+".mzXML"
    guest_mzXML = os.path.join(guest_dirname,gfile)
    host_mzXML = os.path.join(file_dir,gfile)
    print ("copying " + guest_mzXML.replace('/','\\') + " to host")

    cmd_tmp = cmd_copyfrom + ['--target-directory' , host_mzXML,  guest_mzXML.replace('/','\\')]
    print(cmd_tmp)
    output = run_cmd(cmd_tmp)
